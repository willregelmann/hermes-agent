...
class TestRootLevelProviderOverride:
    ...
    def test_model_provider_wins_over_root_provider(self, tmp_path, monkeypatch):
        ...
        import cli
        cfg = cli.load_cli_config()  # resolves HERMES_HOME live
    ...
    def test_terminal_vercel_runtime_bridged_to_env(self, tmp_path, monkeypatch):
        ...
        import cli
        # load_cli_config() is a pure read of the live home; the env bridge is the separate
        # _export_config_to_env step (run once at import), so drive it explicitly here.
        cfg, has_terminal = cli._load_cli_config_impl()
        cli._export_config_to_env(cfg, has_terminal)
