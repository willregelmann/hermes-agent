...
from gateway.platforms import api_server_gateway_restart as _gateway_restart
...
class APIServerAdapter(OpenAICompatRoutesMixin, BasePlatformAdapter):
    ...
    def _http_route_table(self) -> List[tuple]:
        ...
        routes.extend(_gateway_restart._http_routes(self))
        if _CRON_AVAILABLE:
    ...
    async def _enqueue_session_chat(
        ...
        *,
        ...
        async def _run_and_record() -> None:
            ...
            try:
                history = await self._conversation_history_for_session(session_id)
                # Same run kwargs the synchronous path uses, so an accepted turn
                # persists exactly what a waited-for turn would.
                result, _usage = await self._run_agent(
                    conversation_history=history, **ctx["run_kwargs"])
