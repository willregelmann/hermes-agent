...
class HandoffStore:
    ...
    def _rows(self) -> Iterator[Handoff]:
        ...
        with self.path.open("r", encoding="utf-8") as fh:
            for line in fh:
                ...
                try:
                    ...
                    # A corrupt line must not hide the rest of the log. Skipping
                    # is safe because every row is self-contained.
                    continue
    ...
    def stale(self, older_than_s: float, *, now: Optional[float] = None) -> list[Handoff]:
        ...
        return [h for h in self.all_latest()
