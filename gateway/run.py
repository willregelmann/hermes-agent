...
def _bridge_auxiliary_config_to_env(_auxiliary_cfg: dict) -> None:
    ...
    for _task_key in _aux_bridged_keys:
        ...
        # No API_KEY: nothing in the tree reads AUXILIARY_{TASK}_API_KEY, so writing it only
        # leaked the key into os.environ (and every child env) for no consumer.
        for _field, _suffix in (("model", "MODEL"), ("base_url", "BASE_URL")):
...
from gateway.run_peer_completion import GatewayPeerCompletionMixin
from gateway.run_alarms import GatewayAlarmsMixin
...
class GatewayRunner(
    ...
    GatewayAgentCacheMixin, GatewayProfileReconcileMixin, GatewayPeerCompletionMixin, GatewayAlarmsMixin):
