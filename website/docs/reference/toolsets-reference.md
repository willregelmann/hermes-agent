...
| `partners` | `tell_partner` | Hand an intent to a partner (a person or another agent) in their own conversation with this agent, which wakes and acts on it. Only available when `partners` is configured. |
