...
**Only turns a person started.** Background-process completions, peer replies and scheduled heartbeats wake a session by reconstructing the *original* source — same platform, same chat id — so the platform alone cannot tell them from the human message they answer. Those self-injected turns skip the per-platform tier and fall through to `service_tier`: nobody is waiting on them, so premium output speed buys nothing. A session `/fast` override still applies to them, because that one was asked for explicitly. Note that `service_tier: fast` set **globally** has no such carve-out — it means every turn, including machinery.

**Subagents never inherit it.** A `delegate_task` child is background work by construction — nobody watches its token stream, and a backgrounded child outlives the turn that spawned it — so the fast params are dropped from what it inherits, under global `fast` too. Only the exact fast values are stripped (`speed: fast`, `service_tier: priority`); an ordinary `service_tier` such as OpenAI's cheaper `flex` still rides along. Set `delegation.request_overrides` to opt a child in deliberately.
