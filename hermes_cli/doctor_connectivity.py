...
# HERMES_HOME (resolved) -> list. Keyed, not a bare global: the list comes from
# providers.list_providers(), which is profile-scoped (bundled + $HERMES_HOME/plugins +
# entry points), so a bare global lets the first profile's list pin doctor output for
# every other profile served by the same process.
_APIKEY_PROVIDERS_CACHE: dict[str, list] = {}


def _apikey_providers_cache_key() -> str:
    """The HERMES_HOME this call would discover providers under."""
    try:
...
def _apikey_providers_for_home() -> list:
    """The one accessor build_probes() and tests read the keyed cache through."""
    key = _apikey_providers_cache_key()
    if key not in _APIKEY_PROVIDERS_CACHE:
        _APIKEY_PROVIDERS_CACHE[key] = _build_apikey_providers_list()
    return _APIKEY_PROVIDERS_CACHE[key]
...
def build_probes() -> list:
    ...
    apikey_providers = _apikey_providers_for_home()
    return [
        ...
        *((row[0], functools.partial(_probe_apikey_provider, *row)) for row in apikey_providers),
